...
import { fetcher, searchCoins } from '@/lib/coingecko.action';
...
const fetchTrendingCoins = async () => {
  return fetcher<{ coins: TrendingCoin[] }>('/search/trending', {}, 60).then(
    (res) => res.coins || []
  );
};
  ...
  return (
    <CommandItem value={coin.id} onSelect={() => onSelect(coin.id)} className="search-item">
      ...
      <div className="dialog">
        <div className="coin-price">{formatCurrency(coin.data.price ?? 0)}</div>
...
export const SearchModal = () => {
  ...
  const { data: trendingCoins = [] } = useSWR<TrendingCoin[]>(
    'trending-coins',
    fetchTrendingCoins,
    {
      revalidateOnFocus: false,
  ...
  const limitedTrendingCoins = trendingCoins.slice(0, TRENDING_LIMIT);
  const showTrending = !hasQuery && limitedTrendingCoins.length > 0;
